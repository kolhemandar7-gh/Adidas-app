# makeing the adidas sales dashboard by using the streamlit 
...
with col4:
    fig0 = px.bar(df, x="Retailer", y= "TotalSales", labels= {"TotalSales" :"Total Sales{$}"},
...
with view2:
    expander= st.expander("Month wise Sale")
    data= result 
    expander.write(data)
with dwn2:
    st.download_button("Get Data", data= data.to_csv().encode("utf-8"),
                       file_name= "MonthlySales.csv", mime= "test/csv")

st.divider()

result1 = df.groupby(by="State")[["TotalSales", "UnitsSold"]].sum().reset_index()

# add the units sold as a line chart on the y-axis

fig3 = go.Figure()
fig3.add_trace(go.Bar(x= result1["State"], y= result1["TotalSales"], name= "Total Saless"))
fig3.add_trace(go.Scatter(x = result1["State"], y= result1["UnitsSold"], mode= "lines", name= "Units Sold", yaxis= "y2"))

fig3.update_layout(
    title= "👉Total Sales and Unite Sold by State",
...
with view3:
    expander= st.expander("View Data for Sales For Units Sold")
    expander.write(result1)
with dwn3:
    st.download_button("Get Data", data= data.to_csv().encode("utf-8"),
                       file_name="UnitsSold.csv", mime= "text/csv")
st.divider()
...
with view4:
    ...
    expander= st.expander("Total Sales by region and city in Treemap")
    expander.write(result2)
with dwn4:
    st.download_button("Get Data", data=result2.to_csv().encode("utf-8"),
                        file_name= "Region by Totalsale.csv", mime= "text/csv")
